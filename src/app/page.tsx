...
export default async function HomePage() {
  ...
  return <main><header className="shell header"><div className="brand">시라</div><form action={signOut}><button className="logout">로그아웃</button></form></header><section className="shell hero"><span className="eyebrow">SIRA WRITING</span><h1>글쓰기로 돌보는<br />우리의 균형</h1><p>{profile.nickname}님, 반가워요.<br />곧 첫 번째 글쓰기 클럽이 이곳에서 시작됩니다.</p></section><section className="shell empty"><strong>클럽을 준비하고 있어요</strong><p>관리자가 개설한 클럽은 모든 회원이 읽고 자유롭게 글을 쓸 수 있습니다.</p></section></main>;
}
